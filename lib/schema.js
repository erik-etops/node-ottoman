 ...
 * @ignore
 */
Schema.prototype._makeField = function (name, fieldDef) {
  if (!name) {
    throw new Error('Schema field is missing name');
  }
  ...
  if (typeof fieldDef === 'string' ||
    this.context.isModel(fieldDef) ||
    Schema.isCoreType(fieldDef) ||
    this.context.isTypeDef(fieldDef)) {
  ...
  field.type = this.context._parseFieldType(fieldDef);
 ...
 * @param {field} the schema field to add; this may be either an object matching
 * the way ottoman defines fields, or can be a Schema.Field object.
 * @returns {schema}
 */
Schema.prototype.addField = function (field) {
  ...
  if (field instanceof Schema.Field) {
    ...
    schemaField = this._makeField(field.name, field);
  }

  if (this.field(field.name)) {
    throw new Error('Schema already contains a field named ' + field.name);
  }
...
/**
 * Get the Schema.Field instance behind a property name.
 * The name may contain dot notation for chaining, e.g. doc.subDoc.x
