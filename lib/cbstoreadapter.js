...
CbStoreAdapter.prototype._ensureGsiIndices = function(callback) {
    ...
    function waitUntilAllIndexesOnline(callWhenCompleted) {
        var q = 'SELECT name, state FROM system:indexes ' +
            'WHERE (state is missing OR state <> \'online\') AND ' +
            'name in [' +
        ...
        var n1ql = couchbase.N1qlQuery.fromString(q);

        storeAdapter.bucket.query(n1ql, function (err, rows, meta) {
            if (err) {
                return callWhenCompleted(err);
            } else if(rows.length === 0) {
                // Finished.
                return callWhenCompleted();
            }

            console.log('Still building: ' + rows.length + ' ' + 
            JSON.stringify(rows));

            setTimeout(function () {
                console.log('Checking index readiness again....');
                // Call recursively, keep checking.
                waitUntilAllIndexesOnline(callWhenCompleted);
            }, 200);
        });
    }
            ...
            if (proced === queries.length) {
                ...
                storeAdapter.bucket.query(n1ql,
                    function(err, rows, meta) {
                        ...
                        // Build indexes returns immediately, but only starts
                        // the build.  They're not ready yet.  We need to 
                        // guarantee to the caller that the indexes are USABLE.
                        return waitUntilAllIndexesOnline(callback);
                    });
